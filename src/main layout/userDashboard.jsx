...
styleEl.textContent = `
  ...
  .cgo-action-card:hover { transform: translateY(-3px) !important; border-color: rgba(249,115,22,0.4) !important; box-shadow: 0 12px 32px rgba(249,115,22,0.12) !important; }
  .cgo-delivery-row:hover { background: rgba(249,115,22,0.04) !important; }
...
const STATUS_STYLES = {
  ...
  active: { bg: "rgba(249,115,22,0.1)", border: "rgba(249,115,22,0.3)", color: "#f97316", label: "In Progress" },
...
export default function UserDashboard() {
  ...
  const inputStyle = { width: "100%", padding: "11px 14px", borderRadius: "10px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", fontSize: "14px", fontFamily: "'DM Sans', sans-serif", outline: "none", boxSizing: "border-box" };
  ...
  return (
    <div style={{ fontFamily: "'DM Sans', sans-serif", background: "#0a1628", color: "#f0f4ff", minHeight: "100vh", display: "flex" }}>
      ...
      <aside style={{ width: "240px", flexShrink: 0, background: "rgba(255,255,255,0.02)", borderRight: "1px solid rgba(255,255,255,0.06)", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", overflowY: "auto" }}>
        ...
        <nav style={{ padding: "16px 12px", flex: 1 }}>
          ...
          {NAV_ITEMS.map(item => (
            <button key={item.id} onClick={() => setActiveNav(item.id)} className={`cgo-nav-item ${activeNav === item.id ? "active" : ""}`}
              style={{ width: "100%", display: "flex", alignItems: "center", gap: "12px", padding: "11px 12px", borderRadius: "10px", background: "transparent", border: activeNav === item.id ? "none" : "none", borderLeft: activeNav === item.id ? "3px solid #f97316" : "3px solid transparent", color: activeNav === item.id ? "#f97316" : "rgba(255,255,255,0.5)", cursor: "pointer", fontSize: "14px", fontFamily: "'DM Sans', sans-serif", fontWeight: activeNav === item.id ? "600" : "400", transition: "all 0.2s", textAlign: "left", marginBottom: "2px", position: "relative" }}>
        ...
        <div style={{ padding: "16px 20px", borderTop: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "linear-gradient(135deg, #f97316, #ea580c)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: "700", color: "#fff", flexShrink: 0 }}>{USER.avatar}</div>
            <div style={{ flex: 1, overflow: "hidden" }}>
              <p style={{ fontSize: "13px", fontWeight: "600", color: "#fff", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{USER.name}</p>
              <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{USER.studentId}</p>
            </div>
      ...
      <main style={{ flex: 1, overflowY: "auto", padding: "32px 36px", maxWidth: "100%" }}>
          ...
          <div style={{ animation: "cgoFadeUp 0.5s ease forwards" }}>
            ...
            <div style={{ marginBottom: "32px" }}>
              ...
              <h1 style={{ fontSize: "clamp(24px, 3vw, 36px)", fontWeight: "700", color: "#fff", margin: "0 0 6px", fontFamily: "'DM Serif Display', serif" }}>
                Hello, <span style={{ color: "#f97316" }}>{USER.name}!</span>
              </h1>
              <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", margin: 0, fontWeight: "300" }}>What do you need delivered today?</p>
            </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "14px" }}>
                {[
                  ...
                  <button key={i} onClick={card.action} className="cgo-action-card"
                    style={{ background: card.primary ? "rgba(249,115,22,0.1)" : "rgba(255,255,255,0.03)", border: `1px solid ${card.primary ? "rgba(249,115,22,0.3)" : "rgba(255,255,255,0.07)"}`, borderRadius: "16px", padding: "20px", cursor: "pointer", textAlign: "left", transition: "all 0.25s", fontFamily: "'DM Sans', sans-serif" }}>
                    <div style={{ fontSize: "24px", marginBottom: "10px" }}>{card.icon}</div>
                    <p style={{ fontSize: "14px", fontWeight: "600", color: card.primary ? "#f97316" : "#fff", margin: "0 0 4px" }}>{card.label}</p>
                    <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", margin: 0, fontWeight: "300" }}>{card.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginBottom: "32px" }}>
              ...
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", overflow: "hidden" }}>
                {DELIVERIES.slice(0, 3).map((d, i) => {
                  ...
                  return (
                    <div key={i} className="cgo-delivery-row" style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px 20px", borderBottom: i < 2 ? "1px solid rgba(255,255,255,0.05)" : "none", transition: "background 0.2s" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)", margin: 0 }}>{d.from} → {d.to}</p>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        ...
                        <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.3)" }}>{d.date}</span>
                      </div>
                    </div>
                  );
                })}
            ...
            <div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
                <h2 style={{ fontSize: "16px", fontWeight: "600", color: "#fff", margin: 0 }}>Riders Near You</h2>
                <span style={{ fontSize: "12px", color: "rgba(255,255,255,0.35)" }}>{RIDERS.filter(r => r.active).length} available now</span>
              </div>
              <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", overflow: "hidden", position: "relative", height: "280px" }}>

                {/* Map background */}
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, #0d2137 0%, #0a1628 50%, #0f1f35 100%)" }}>
                  {/* Grid lines */}
                  {[...Array(8)].map((_, i) => (
                    <div key={`h${i}`} style={{ position: "absolute", left: 0, right: 0, top: `${i * 14}%`, height: "1px", background: "rgba(255,255,255,0.04)" }} />
                  ))}
                  {[...Array(10)].map((_, i) => (
                    <div key={`v${i}`} style={{ position: "absolute", top: 0, bottom: 0, left: `${i * 11}%`, width: "1px", background: "rgba(255,255,255,0.04)" }} />
                  ))}
                  {/* Roads */}
                  <div style={{ position: "absolute", top: "45%", left: 0, right: 0, height: "2px", background: "rgba(249,115,22,0.1)" }} />
                  <div style={{ position: "absolute", left: "40%", top: 0, bottom: 0, width: "2px", background: "rgba(249,115,22,0.1)" }} />
                  <div style={{ position: "absolute", top: "70%", left: "20%", right: "10%", height: "1px", background: "rgba(255,255,255,0.06)" }} />
                  <div style={{ position: "absolute", left: "65%", top: "20%", bottom: "30%", width: "1px", background: "rgba(255,255,255,0.06)" }} />
                </div>
                ...
                <div style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%, -50%)", zIndex: 5 }}>
                  ...
                  <p style={{ position: "absolute", top: "18px", left: "50%", transform: "translateX(-50%)", fontSize: "10px", color: "#fff", whiteSpace: "nowrap", background: "rgba(10,22,40,0.8)", padding: "2px 6px", borderRadius: "4px" }}>You</p>
                </div>
                ...
                {RIDERS.map(r => (
                  <div key={r.id} className="cgo-rider-pin" onClick={() => setSelectedRider(selectedRider?.id === r.id ? null : r)}
                    ...
                    <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: r.active ? "linear-gradient(135deg, #f97316, #ea580c)" : "rgba(255,255,255,0.2)", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", boxShadow: r.active ? "0 4px 12px rgba(249,115,22,0.4)" : "none" }}>
                      🏍️
                    </div>
                    {/* Rider tooltip */}
                    {selectedRider?.id === r.id && (
                      <div style={{ position: "absolute", bottom: "38px", left: "50%", transform: "translateX(-50%)", background: "rgba(10,22,40,0.95)", border: "1px solid rgba(249,115,22,0.3)", borderRadius: "10px", padding: "10px 14px", whiteSpace: "nowrap", zIndex: 10 }}>
                        <p style={{ fontSize: "13px", fontWeight: "600", color: "#fff", margin: "0 0 2px" }}>{r.name}</p>
                        <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", margin: "0 0 2px" }}>⭐ {r.rating} · {r.vehicle}</p>
                        <p style={{ fontSize: "11px", color: "#f97316", margin: 0 }}>{r.distance} away</p>
                      </div>
                    )}
                  </div>
                ))}
                ...
                <div style={{ position: "absolute", bottom: "12px", left: "12px", display: "flex", gap: "12px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "5px", background: "rgba(10,22,40,0.8)", padding: "5px 10px", borderRadius: "8px" }}>
                    <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#f97316" }} />
                    <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.6)" }}>Available</span>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "5px", background: "rgba(10,22,40,0.8)", padding: "5px 10px", borderRadius: "8px" }}>
                    <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "rgba(255,255,255,0.3)" }} />
                    <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.6)" }}>Busy</span>
                  </div>
                </div>

                {/* Tap hint */}
                <div style={{ position: "absolute", bottom: "12px", right: "12px", background: "rgba(10,22,40,0.8)", padding: "5px 10px", borderRadius: "8px" }}>
                  <span style={{ fontSize: "11px", color: "rgba(255,255,255,0.4)" }}>Tap a rider to preview</span>
                </div>
        ...
        {activeNav === "request" && (
          <div style={{ animation: "cgoFadeUp 0.5s ease forwards", maxWidth: "560px" }}>
            <h1 style={{ fontSize: "28px", fontWeight: "700", color: "#fff", marginBottom: "6px", fontFamily: "'DM Serif Display', serif" }}>Request a Delivery</h1>
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", marginBottom: "32px", fontWeight: "300" }}>Fill in the details and a rider will be assigned to you.</p>
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "20px", padding: "32px" }}>
              <div style={{ display: "flex", flexDirection: "column", gap: "18px" }}>
                <div>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px", fontWeight: "500", letterSpacing: "0.3px" }}>Item Name *</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px", fontWeight: "500", letterSpacing: "0.3px" }}>Item Description</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px", fontWeight: "500", letterSpacing: "0.3px" }}>Pickup Location *</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px", fontWeight: "500", letterSpacing: "0.3px" }}>Dropoff Location *</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.5)", marginBottom: "8px", fontWeight: "500", letterSpacing: "0.3px" }}>Note for Rider</label>
                ...
                {/* Fee estimate */}
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 16px", background: "rgba(249,115,22,0.07)", border: "1px solid rgba(249,115,22,0.15)", borderRadius: "12px" }}>
                  <div>
                    <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", margin: "0 0 2px" }}>Estimated Fee</p>
                    <p style={{ fontSize: "18px", fontWeight: "700", color: "#f97316", margin: 0 }}>GH₵ 6 — 12</p>
                  </div>
                  <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.3)", margin: 0, textAlign: "right" }}>Based on<br />distance</p>
                </div>

                <button onClick={handleRequestSubmit} className="cgo-btn-primary"
                  style={{ background: "#f97316", border: "none", color: "#fff", padding: "14px", borderRadius: "12px", fontSize: "15px", fontWeight: "600", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", boxShadow: "0 6px 20px rgba(249,115,22,0.3)", transition: "all 0.25s" }}>
        ...
        {activeNav === "track" && (
          <div style={{ animation: "cgoFadeUp 0.5s ease forwards" }}>
            <h1 style={{ fontSize: "28px", fontWeight: "700", color: "#fff", marginBottom: "6px", fontFamily: "'DM Serif Display', serif" }}>Track Your Delivery</h1>
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", marginBottom: "28px", fontWeight: "300" }}>Follow your rider in real time.</p>

            {/* Active delivery card */}
            <div style={{ background: "rgba(249,115,22,0.07)", border: "1px solid rgba(249,115,22,0.2)", borderRadius: "20px", padding: "24px", marginBottom: "24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
                <div>
                  <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", margin: "0 0 4px", textTransform: "uppercase", letterSpacing: "0.8px" }}>Active Delivery</p>
                  <p style={{ fontSize: "16px", fontWeight: "600", color: "#fff", margin: 0 }}>📦 Lecture Notes + Charger</p>
                </div>
                <span style={{ fontSize: "12px", fontWeight: "600", color: "#f97316", background: "rgba(249,115,22,0.1)", border: "1px solid rgba(249,115,22,0.3)", padding: "5px 12px", borderRadius: "100px" }}>In Progress</span>
              </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "8px", marginBottom: "20px" }}>
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                  ...
                  <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", margin: 0 }}>Unity Hall, Room 214</p>
                </div>
                ...
                <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                  ...
                  <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.6)", margin: 0 }}>Main Library, Study Room B</p>
                </div>
              </div>
              ...
              <div style={{ marginBottom: "20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", color: "rgba(255,255,255,0.4)", marginBottom: "8px" }}>
                  ...
                  <span style={{ color: "#f97316", fontWeight: "600", animation: "cgoBlink 1.5s ease-in-out infinite" }}>~8 mins away</span>
                </div>
                <div style={{ height: "6px", background: "rgba(255,255,255,0.08)", borderRadius: "6px", overflow: "hidden" }}>
                  <div style={{ height: "100%", width: "60%", background: "linear-gradient(90deg, #f97316, #fb923c)", borderRadius: "6px" }} />
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: "rgba(255,255,255,0.25)", marginTop: "6px" }}>
                  <span>✅ Picked up</span>
                  <span style={{ color: "#f97316" }}>🏍️ On the way</span>
                  <span>📦 Delivered</span>
                </div>
              </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "12px", padding: "14px", background: "rgba(255,255,255,0.04)", borderRadius: "12px" }}>
                <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "linear-gradient(135deg, #f97316, #ea580c)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", fontWeight: "700", color: "#fff" }}>KO</div>
                <div style={{ flex: 1 }}>
                  <p style={{ fontSize: "14px", color: "#fff", margin: "0 0 2px", fontWeight: "600" }}>Kofi Owusu</p>
                  <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.4)", margin: 0 }}>⭐ 4.9 · Honda CB125</p>
                </div>
            ...
            {/* Mini map */}
            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "16px", overflow: "hidden", height: "260px", position: "relative" }}>
              <div style={{ position: "absolute", inset: 0, background: "linear-gradient(135deg, #0d2137, #0a1628)" }}>
                {[...Array(8)].map((_, i) => <div key={i} style={{ position: "absolute", left: 0, right: 0, top: `${i * 14}%`, height: "1px", background: "rgba(255,255,255,0.04)" }} />)}
                {[...Array(10)].map((_, i) => <div key={i} style={{ position: "absolute", top: 0, bottom: 0, left: `${i * 11}%`, width: "1px", background: "rgba(255,255,255,0.04)" }} />)}
                <div style={{ position: "absolute", top: "45%", left: 0, right: 0, height: "2px", background: "rgba(249,115,22,0.1)" }} />
                <div style={{ position: "absolute", left: "40%", top: 0, bottom: 0, width: "2px", background: "rgba(249,115,22,0.1)" }} />
              </div>
              {/* User */}
              ...
              <div style={{ position: "absolute", top: "40%", left: "45%", transform: "translate(-50%,-50%)", zIndex: 5, animation: "cgoRiderMove 4s ease-in-out infinite" }}>
                <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "linear-gradient(135deg, #f97316, #ea580c)", border: "2px solid #fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "14px", boxShadow: "0 4px 12px rgba(249,115,22,0.4)" }}>🏍️</div>
              </div>
              <div style={{ position: "absolute", bottom: "12px", left: "12px", background: "rgba(10,22,40,0.85)", padding: "6px 12px", borderRadius: "8px" }}>
                <span style={{ fontSize: "12px", color: "#f97316", fontWeight: "600", animation: "cgoBlink 1.5s ease-in-out infinite" }}>● Live tracking active</span>
              </div>
            </div>
          </div>
        )}
        ...
        {activeNav === "history" && (
          <div style={{ animation: "cgoFadeUp 0.5s ease forwards" }}>
            ...
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", marginBottom: "28px", fontWeight: "300" }}>All your past deliveries in one place.</p>

            <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", overflow: "hidden" }}>
              {DELIVERIES.map((d, i) => {
                const s = STATUS_STYLES[d.status];
                return (
                  <div key={i} className="cgo-delivery-row" style={{ display: "flex", alignItems: "center", gap: "16px", padding: "18px 24px", borderBottom: i < DELIVERIES.length - 1 ? "1px solid rgba(255,255,255,0.05)" : "none", transition: "background 0.2s" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: "12px", color: "rgba(255,255,255,0.25)", margin: 0 }}>{d.rider !== "—" ? `Rider: ${d.rider}` : "No rider assigned"} · {d.date}</p>
                    </div>
                    <div style={{ textAlign: "right", flexShrink: 0 }}>
                      ...
                      <p style={{ fontSize: "13px", fontWeight: "600", color: "#f97316", margin: "0 0 4px" }}>{d.fee}</p>
                      {d.rating && (
                        <p style={{ fontSize: "11px", color: "rgba(255,255,255,0.35)", margin: 0 }}>{"⭐".repeat(d.rating)}</p>
                      )}
                    </div>
                  </div>
                );
        ...
        {activeNav === "profile" && (
          <div style={{ animation: "cgoFadeUp 0.5s ease forwards", maxWidth: "560px" }}>
            ...
            <p style={{ fontSize: "14px", color: "rgba(255,255,255,0.4)", marginBottom: "28px", fontWeight: "300" }}>Manage your account details.</p>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "20px", padding: "24px", background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", marginBottom: "20px" }}>
              <div style={{ width: "64px", height: "64px", borderRadius: "50%", background: "linear-gradient(135deg, #f97316, #ea580c)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", fontWeight: "700", color: "#fff" }}>{USER.avatar}</div>
              <div>
                <p style={{ fontSize: "20px", fontWeight: "700", color: "#fff", margin: "0 0 4px", fontFamily: "'DM Serif Display', serif" }}>{USER.name}</p>
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.4)", margin: "0 0 2px" }}>{USER.email}</p>
                <p style={{ fontSize: "13px", color: "rgba(255,255,255,0.3)", margin: 0 }}>Student ID: {USER.studentId}</p>
              </div>
            </div>
      ...
      {/* ── REQUEST MODAL ── */}
      {showRequestModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", backdropFilter: "blur(6px)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
          <div style={{ background: "#0d1f35", border: "1px solid rgba(249,115,22,0.2)", borderRadius: "24px", padding: "32px", width: "100%", maxWidth: "480px", animation: "cgoFadeUp 0.3s ease forwards" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
              <h2 style={{ fontSize: "20px", fontWeight: "700", color: "#fff", margin: 0, fontFamily: "'DM Serif Display', serif" }}>Request a Delivery</h2>
              <button onClick={() => setShowRequestModal(false)} style={{ background: "transparent", border: "none", color: "rgba(255,255,255,0.4)", cursor: "pointer", fontSize: "20px" }}>✕</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
              {[
                ...
                <div key={field.name}>
                  <label style={{ display: "block", fontSize: "12px", color: "rgba(255,255,255,0.45)", marginBottom: "6px", fontWeight: "500" }}>{field.label}</label>
              ...
              <button onClick={handleRequestSubmit} className="cgo-btn-primary"
                style={{ background: "#f97316", border: "none", color: "#fff", padding: "13px", borderRadius: "12px", fontSize: "15px", fontWeight: "600", cursor: "pointer", fontFamily: "'DM Sans', sans-serif", marginTop: "6px", transition: "all 0.25s" }}>
